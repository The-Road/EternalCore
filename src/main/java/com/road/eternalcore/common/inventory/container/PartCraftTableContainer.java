...
import com.road.eternalcore.common.inventory.container.slot.PartCraftTableResultSlot;
...
public class PartCraftTableContainer extends Container {
    ...
    public PartCraftTableContainer(int containerId, PlayerInventory inventory) {
        this(containerId, inventory, IWorldPosCallable.NULL);
    }
    public PartCraftTableContainer(int containerId, PlayerInventory inventory, IWorldPosCallable posCallable) {
        super(ModContainerType.partCraftTable, containerId);
        ...
        addSlot(new PartCraftTableResultSlot(player, inputSlots, resultSlot, byProductSlot, 0, 107, 48));
    ...
    protected static void slotChangedCraftingGrid(int containerId, World world, PlayerEntity player, PartCraftTableInventory inputSlots, CraftResultInventory resultSlot) {
        if (!world.isClientSide()) {
            ServerPlayerEntity serverplayerentity = (ServerPlayerEntity)player;
            ItemStack itemstack = ItemStack.EMPTY;
            Optional<PartCraftingRecipe> recipeOptional = world.getServer().getRecipeManager().getRecipeFor(ModRecipeType.PART_CRAFTING, inputSlots, world);
            if (recipeOptional.isPresent()){
                PartCraftingRecipe recipe = recipeOptional.get();
    ...
    public void removed(PlayerEntity player) {
        ...
        this.access.execute((world, blockPos) -> {
            ...
            this.clearContainer(player, world, this.byProductSlot);
        });
    }
